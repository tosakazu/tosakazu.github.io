...
(function () {
  ...
  /**
   * アプリ内ブラウザ (X / Discord 等) なら警告を出す。
   * 認証の途中で start.gg アプリに移り、戻り先が通常の Safari になるため、
   * 始めたブラウザと戻ったブラウザが別物になって必ず失敗する。
   * 保存領域の共有では直せないので、始める前に気づいてもらう。
   */
  function updateInAppWarning() {
    if (!els.inapp) return;
    var app = S.inAppBrowser(navigator.userAgent);
    els.inapp.hidden = !app;
    if (app && els.inappApp) els.inappApp.textContent = app;
  }

  function copyPageUrl() {
    var url = CFG.CANONICAL_ORIGIN + CFG.CANONICAL_BASE + 'vote.html';
    var done = function () {
      if (!els.copyDone) return;
      els.copyDone.hidden = false;
      setTimeout(function () { els.copyDone.hidden = true; }, 3000);
    };
    try {
    ...
    } catch (_) { /* コピーできなくても URL は画面に出ている */ }
  }
  ...
  function init() {
    els = {
      ...
      inapp: $('vt-inapp'),
      inappApp: $('vt-inapp-app'),
      copyUrl: $('vt-copy-url'),
      copyDone: $('vt-copy-done'),
    };
    ...
    if (els.copyUrl) els.copyUrl.addEventListener('click', copyPageUrl);
